...
export default function ServicesCard({ userData, updateUserData }) {
  ...
  // Add this function to update negotiable status:
  const updateServiceNegotiable = (serviceName, isNegotiable) => {
    setSelectedServices((prev) =>
  ...
  const handleServiceImageSelect = (serviceName, file) => {
    if (!file) return
    ...
    reader.onloadend = () => {
      setImagePreviews((prev) => ({ ...prev, [serviceName]: reader.result }))
    }
  ...
  const updateServicePricing = (serviceName, field, value) => {
    ...
    try {
      ...
      if (userData?.userType === "spa") {
        Object.entries(serviceImages).forEach(([serviceName, file]) => {
          formData.append(`image_${serviceName}`, file)
        })
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      requestFormData.append("isActive", formData.isActive)
      requestFormData.append("price", formData.price)
      requestFormData.append("pricingUnit", formData.pricingUnit)
      requestFormData.append("contactForPrice", formData.contactForPrice)
      requestFormData.append("priceNegotiable", formData.priceNegotiable);
              ...
              >
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-medium ${service.isActive ? "bg-success/20 text-success" : "bg-neutral-700 text-text-muted"
                        }`}
                    >
                  ...
                  <div className="mb-2">
                    {service.contactForPrice ? (
                      ...
                      <div className="mb-2">
                        {service.contactForPrice ? (
                          <span className="text-orange-600 font-medium">Contact for Price</span>
                        ) : (
                          <div className="flex items-center gap-2">
                            <span className="text-primary font-bold">
                              KSh {service.price} <span className="text-text-muted text-sm font-medium">/ {service.pricingUnit}</span>
                            </span>
                            ...
                            {!service.priceNegotiable && service.price > 0 && (
                ...
                <div className="flex items-center gap-2 max-md:flex-col">
                  ...
                  >
                    <div
                      className={`w-10 h-6 rounded-full transition-colors ${service.isActive ? "bg-primary" : "bg-neutral-700"
                        } relative`}
                    >
                      <div
                        className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${service.isActive ? "translate-x-5" : "translate-x-1"
                          }`}
                      />
                    </div>
                  </button>
              ...
              {editingService ? (
                <div className="space-y-4">
                  ...
                  {userData?.userType === "spa" && (
                    <div>
                      <label className="block text-sm font-medium text-text-primary mb-2">Service Image</label>
                      <div className="flex items-center gap-4">
                        {imagePreviews[formData.name] || editingService?.image?.url ? (
                          <div className="relative">
                            <img
                              src={imagePreviews[formData.name] || editingService.image.url}
                              alt="Service preview"
                            ...
                            <button
                              onClick={() => {
                                const newImages = { ...serviceImages }
                                const newPreviews = { ...imagePreviews }
                                delete newImages[formData.name]
                                delete newPreviews[formData.name]
                                setServiceImages(newImages)
                                setImagePreviews(newPreviews)
                              }}
                        ...
                        <label className="cursor-pointer">
                          <input
                            type="file"
                            accept="image/*"
                            onChange={(e) => handleServiceImageSelect(formData.name, e.target.files[0])}
                            className="hidden"
                          />
                          <div className="px-4 py-2 bg-bg-primary border border-border-light rounded-lg text-text-primary hover:bg-neutral-800 transition-colors flex items-center gap-2">
                            <AiOutlineCamera />
                            {imagePreviews[formData.name] || editingService?.image?.url ? "Change Image" : "Add Image"}
                          </div>
                        </label>
                      </div>
                      <p className="text-xs text-text-muted mt-1">Max 5MB, recommended 800x600px</p>
                    </div>
                  )} */}
                      ...
                      {COMMON_SERVICES.map((service) => {
                        ...
                        return (
                          <button
                            ...
                            className={`px-4 py-3 rounded-lg text-sm font-medium transition-all border-2 cursor-pointer ${isSelected
                    ...
                    <div className="space-y-4 border-t border-border-light pt-6">
                      ...
                      {selectedServices.map((service) => (
                        ...
                        >
                          ...
                          {/* {userData?.userType === "spa" && (
                            <div>
                              {imagePreviews[service.name] ? (
                                <div className="relative inline-block">
                                  <img
                                    src={imagePreviews[service.name] || "/placeholder.svg"}
                                  ...
                                  <button
                                    onClick={() => {
                                      const newImages = { ...serviceImages }
                                      const newPreviews = { ...imagePreviews }
                                      delete newImages[service.name]
                                      delete newPreviews[service.name]
                                      setServiceImages(newImages)
                                      setImagePreviews(newPreviews)
                                    }}
                                    className="absolute -top-2 -right-2 bg-error text-white rounded-full p-1.5"
                                  >
                                    <AiOutlineDelete className="text-sm" />
                                  </button>
                                </div>
                                ...
                                <label className="cursor-pointer">
                                  <input
                                    type="file"
                                    accept="image/*"
                                    onChange={(e) => handleServiceImageSelect(service.name, e.target.files[0])}
                                    className="hidden"
                                  />
                                  <div className="px-3 py-2 bg-bg-secondary border border-border-light rounded-lg text-text-primary hover:bg-neutral-800 transition-colors flex items-center gap-2 text-sm">
                                    <AiOutlineCamera />
                                    Add Image (Optional)
                                  </div>
                                </label>
